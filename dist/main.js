...
module.exports = {
    ...
    init: function init() {
        ...
        /*opacity sun*/
        var opSun = new THREE.Mesh(new THREE.SphereGeometry(14, 28, 28), new THREE.MeshLambertMaterial({
            color: 0xff0000,
            // emissive: 0xdd4422,
            transparent: true,
            opacity: .35
        ...
        var sunLight = new THREE.PointLight(0xddddaa, 1.5);
        scene.add(sunLight);
    ...
    initPlanet: function initPlanet(name, speed, angle, color, distance, volume, ringMsg) {
        ...
        var star = {
            ...
            var ring = new THREE.Mesh(new THREE.RingGeometry(ringMsg.innerRadius, ringMsg.outerRadius, 32, 6), new THREE.MeshBasicMaterial({
                color: ringMsg.color,
    ...
    initEarth: function initEarth(name, speed, angle, distance, volume) {
        var texture = THREE.ImageUtils.loadTexture('../resource/img/earth.png');
        var mat = new THREE.MeshPhongMaterial();
        mat.map = texture; //材质的Map属性用于添加纹理
        var mesh = new THREE.Mesh(new THREE.SphereGeometry(volume, 16, 16), mat);
        ...
        var track = new THREE.Mesh(new THREE.RingGeometry(distance - 0.2, distance + 0.2, 64, 1), new THREE.MeshBasicMaterial({ color: 0x888888, side: THREE.DoubleSide }));
        track.rotation.x = -Math.PI / 2;
        track.rotation.y = track.rotation.y === 2 * Math.PI ? 0.0001 * Math.PI : track.rotation.y + 0.0001 * Math.PI;
    ...
    initPlantWithSkin: function initPlantWithSkin(name, speed, angle, distance, volume, skinUrl) {
    ...
    displayPlanetName: function displayPlanetName() {
        stars.forEach(function (star) {
            return nameConstructor(star.name, stars.volume);
        });
